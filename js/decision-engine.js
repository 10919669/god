...
import { enhanceResultContent } from './result-experience.js';
...
function pickTemplateBundle(tpl) {
  const titles = Array.isArray(tpl.title) ? tpl.title : [tpl.title || ''];
  const index = Math.floor(Math.random() * Math.max(titles.length, 1));
  const pickAligned = (field) => {
    const list = Array.isArray(tpl[field]) ? tpl[field] : [tpl[field] || ''];
    return list[index] ?? pick(list);
  };

  return {
    resultTitle: titles[index] || pick(titles),
    resultReason: pickAligned('reason'),
    resultAction: pickAligned('action'),
...
const TEMPLATES = {
  ...
  wear_today: {
    ...
    // 风格3-休闲舒适（wait/lean_B/B）→ 统一用黄色/亮色休闲图，避免无图颜色错配
    casual: {
      title: [
        '今天穿黄色',
        '穿亮色休闲',
...
export function translateToDisplay(decision, question, category, seriousnessLevel, toneMode, templateId, date, forceTone) {
  ...
  if (templateId === 'eat_today') {
    ...
    if (!tpl) {
      ...
      ({ resultTitle, resultReason, resultAction, resultTrend } = pickTemplateBundle(fallbackTpl));
    } else {
      ({ resultTitle, resultReason, resultAction, resultTrend } = pickTemplateBundle(tpl));
    }
    // 每日幸运彩蛋（基于日期的确定性随机）
...
export function getDecision(question, templateId, date = new Date(), geoPermission = 'denied', city = null) {
  ...
  return decorateStructuredResult(
    baseResult,
    question,
    seriousnessLevel === 'L3' ? 'oracle' : 'bestie'
  );
}
...
export function retranslate(result, forceTone) {
  ...
  return decorateStructuredResult(
    ...
    result.questionText || '',
    forceTone || result.tone || (result.seriousnessLevel === 'L3' ? 'oracle' : 'bestie')
  );
}

function decorateStructuredResult(result, questionText, tone) {
  ...
  const structured = enhanceResultContent(
    {
      ...result,
      questionText,
      tone,
